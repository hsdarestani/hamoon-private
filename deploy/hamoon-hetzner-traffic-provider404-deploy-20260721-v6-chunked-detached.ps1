...
$GeneratedPath = Join-Path $env:TEMP "hamoon-hetzner-traffic-provider404-deploy-20260721-v6-generated-$PID.ps1"
...
Write-Host "V6_PRODUCTION_PREFLIGHT=SUCCESS" -ForegroundColor Green
...
$RemoteBase64Path = "/tmp/hamoon-traffic-provider404-v6-$UploadId.b64"
$RemoteScriptPath = "/tmp/hamoon-traffic-provider404-v6-$UploadId.sh"
$RemoteStatusPath = "/tmp/hamoon-traffic-provider404-v6-$UploadId.status"
$RemotePidPath = "/tmp/hamoon-traffic-provider404-v6-$UploadId.pid"
$RemoteLogPath = "/root/hamoon-traffic-provider404-v6-$UploadId.log"
$ChunkSize = 2048
$ChunkCount = [Math]::Ceiling($RemoteBase64.Length / $ChunkSize)
...
for ($Index = 0; $Index -lt $RemoteBase64.Length; $Index += $ChunkSize) {
    ...
    $Number = [Math]::Floor($Index / $ChunkSize) + 1
...
$FinalCheckCommand = "grep -q 'HAMOON_HETZNER_TRAFFIC_PROVIDER404_V6=SUCCESS' '$RemoteLogPath' && grep -q 'FINAL_DASHBOARD_HTTP=200' '$RemoteLogPath' && cd /root/Hamoon && node --check index.js && node --check cloud-api.js && node --check Hetzner/hetzner-api.js && echo FINAL_REMOTE_VERIFICATION_V6=SUCCESS"
$FinalText = (Invoke-HamoonSshSmall -Command $FinalCheckCommand -Retries 8) -join "`n"
Write-Host $FinalText
if ($FinalText -notlike "*FINAL_REMOTE_VERIFICATION_V6=SUCCESS*") {
...
Write-Host ""
Write-Host "CHUNKED_SSH_TRANSPORT_V6=SUCCESS" -ForegroundColor Green
Write-Host "HETZNER TRAFFIC + PROVIDER-404 HOTFIX v6 DEPLOYMENT COMPLETED" -ForegroundColor Green
'@
'@
...
$Patched = $Patched.Replace(
    ...
    'hamoon-hetzner-traffic-provider404-deploy-20260721-v6-generated-'
)
$Patched = $Patched.Replace('GUARD_V5', 'GUARD_V6')
$Patched = $Patched.Replace('TRANSPORT_V5', 'TRANSPORT_V6')
$Patched = $Patched.Replace('HOTFIX v5', 'HOTFIX v6')
$Patched = $Patched.Replace('PROVIDER404_V5', 'PROVIDER404_V6')
$Patched = $Patched.Replace('GENERATED_V5_', 'GENERATED_V6_')
$Patched = $Patched.Replace('Generated v5', 'Generated v6')
$Patched = $Patched.Replace('generated v5', 'generated v6')

foreach ($Marker in @(
    "MANAGEMENT_PROVIDER404_GUARD_V6",
    "CHUNKED_SSH_TRANSPORT_V6=SUCCESS",
    "HAMOON_HETZNER_TRAFFIC_PROVIDER404_V6=SUCCESS",
    "V6_PRODUCTION_PREFLIGHT=SUCCESS"
)) {
    if (-not $Patched.Contains($Marker)) {
        throw "Generated v6 verification failed for marker: $Marker"
    }
}
...
if ($ParseErrors.Count -gt 0) {
    ...
    throw "Generated v6 PowerShell parser check failed: $Details"
}

Write-Host "GENERATED_V6_POWERSHELL_PARSE_CHECK=SUCCESS"
...
try {
    ...
    Write-Host ""
    Write-Host "===== GENERATED CHUNKED/DETACHED TRAFFIC HOTFIX v6 =====" -ForegroundColor Cyan
    Write-Host "Source:    $V5Path"
    Write-Host "Generated: $GeneratedPath"
    Write-Host "CHUNKED_SSH_TRANSPORT_V6=GENERATED"
    Write-Host "DETACHED_REMOTE_EXECUTION_V6=GENERATED"
