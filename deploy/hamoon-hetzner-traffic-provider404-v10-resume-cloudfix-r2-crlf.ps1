...
$PriorStatusPath = '/tmp/hamoon-traffic-provider404-v10-resume-20260722113845-15072.status'
$PriorLogPath = '/root/hamoon-traffic-provider404-v10-resume-20260722113845-15072.log'
$ExpectedRemoteScriptSha256 = '0e492f4ce583c0176b317f9ea2d222b52f7ef33b62185c4fc42b5e1dc8cb39ec'
...
Write-Host ''
Write-Host '===== VERIFY FAILED CRLF-SENSITIVE RESUME AND PRODUCTION =====' -ForegroundColor Cyan

$ForensicsCommand = "test -f '$RemoteScriptPath' && test -f '$PriorStatusPath' && test -f '$PriorLogPath' && printf 'PRIOR_STATUS=' && cat '$PriorStatusPath' && echo && sha256sum '$RemoteScriptPath' && grep -q 'CLOUD_GET_SERVER_EXPORT_EXACT_ANCHOR_NOT_FOUND' '$PriorLogPath' && grep -q 'AUTOMATIC_TRAFFIC_PROVIDER404_ROLLBACK=COMPLETED' '$PriorLogPath' && echo PRIOR_CRLF_FAILURE=VERIFIED"
...
if ($Forensics.StdOut -notlike '*PRIOR_STATUS=1*' -or
    ...
    $Forensics.StdOut -notlike '*PRIOR_CRLF_FAILURE=VERIFIED*') {
    throw 'The prior failed resume artifacts do not match the verified CRLF-anchor failure. Nothing was changed.'
}
...
Write-Host 'V10_R2_PREFLIGHT=SUCCESS' -ForegroundColor Green

Write-Host ''
Write-Host '===== PATCH CRLF NORMALIZATION INTO TEMPORARY v10 SCRIPT =====' -ForegroundColor Cyan
...
marker = 'CLOUD_EXPORT_MULTILINE_FIX_V10R2'

if marker in text:
    print('REMOTE_V10_CRLF_FIX=ALREADY_APPLIED')
    raise SystemExit(0)

needle = """    # CLOUD_EXPORT_MULTILINE_FIX_V10R1
    export_start = cloud.find('module.exports = {')
"""
replacement = """    # CLOUD_EXPORT_MULTILINE_FIX_V10R2
    cloud = cloud.replace('\\r\\n', '\\n').replace('\\r', '\\n')
    export_start = cloud.find('module.exports = {')
"""
...
if count != 1:
    raise SystemExit(f'EXPECTED_ONE_V10R1_MARKER_FOUND_{count}')

fixed = text.replace(needle, replacement, 1)
path.write_text(fixed, encoding='utf-8')
print('REMOTE_V10_CRLF_FIX=APPLIED')
'@

$PatcherBase64 = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($RemotePatcher))
$PatchCommand = "printf '%s' '$PatcherBase64' | base64 -d | python3 - '$RemoteScriptPath' && bash -n '$RemoteScriptPath' && grep -q 'CLOUD_EXPORT_MULTILINE_FIX_V10R2' '$RemoteScriptPath' && echo REMOTE_V10_R2_SCRIPT_VERIFY=SUCCESS && sha256sum '$RemoteScriptPath'"
$PatchResult = Invoke-HamoonSsh -Command $PatchCommand -Retries 10
Write-Host $PatchResult.StdOut.Trim()

if ($PatchResult.StdOut -notlike '*REMOTE_V10_R2_SCRIPT_VERIFY=SUCCESS*') {
    throw 'The temporary v10 script could not be patched and verified. Production was not changed.'
}

Write-Host ''
Write-Host '===== LAUNCH CRLF-FIXED v10 AS DETACHED RUN =====' -ForegroundColor Cyan

$RunId = "$(Get-Date -Format 'yyyyMMddHHmmss')-$PID"
$RunLogPath = "/root/hamoon-traffic-provider404-v10-r2-$RunId.log"
$RunStatusPath = "/tmp/hamoon-traffic-provider404-v10-r2-$RunId.status"
$RunPidPath = "/tmp/hamoon-traffic-provider404-v10-r2-$RunId.pid"

$LaunchCommand = "if [ -f '$RunStatusPath' ]; then echo R2_STATUS=`$(cat '$RunStatusPath'); elif [ -f '$RunPidPath' ] && kill -0 `$(cat '$RunPidPath') 2>/dev/null; then echo R2_RUNNING=`$(cat '$RunPidPath'); else rm -f '$RunLogPath' '$RunStatusPath' '$RunPidPath'; nohup sh -c 'bash $RemoteScriptPath >$RunLogPath 2>&1; echo `$? >$RunStatusPath' </dev/null >/dev/null 2>&1 & echo `$! >'$RunPidPath'; echo R2_STARTED=`$(cat '$RunPidPath'); fi"
$LaunchResult = Invoke-HamoonSsh -Command $LaunchCommand -Retries 10
Write-Host $LaunchResult.StdOut.Trim()
Write-Host "R2_REMOTE_LOG=$RunLogPath"
Write-Host "R2_REMOTE_STATUS=$RunStatusPath"

if ($LaunchResult.StdOut -notlike '*R2_STARTED=*' -and
    $LaunchResult.StdOut -notlike '*R2_RUNNING=*' -and
    $LaunchResult.StdOut -notlike '*R2_STATUS=*') {
    throw 'The detached r2 deployment was not confirmed. Do not rerun before checking the printed paths.'
}
...
while ((Get-Date) -lt $Deadline) {
    ...
    Write-Host 'R2_DEPLOY_STATUS=RUNNING'
}

if ($null -eq $RemoteExitCode) {
    throw "The detached r2 deployment is still running or status could not be read. Do not rerun. Log: $RunLogPath"
}

Write-Host "R2_DEPLOY_EXIT_CODE=$RemoteExitCode"
...
Write-Host ''
Write-Host '===== R2 DEPLOYMENT MARKERS =====' -ForegroundColor Cyan
Write-Host $MarkerResult.StdOut.Trim()

if ($RemoteExitCode -ne 0) {
    $TailResult = Invoke-HamoonSsh -Command "tail -n 180 '$RunLogPath'" -Retries 8
    Write-Host ''
    Write-Host '===== R2 FAILURE LOG TAIL =====' -ForegroundColor Red
    Write-Host $TailResult.StdOut
    throw "CRLF-fixed v10 deployment failed with exit code $RemoteExitCode. Review rollback markers above."
}
...
if ($RequiredMarkerResult.StdOut -notlike '*R2_REQUIRED_MARKERS=SUCCESS*') {
    throw 'Remote process exited zero but required success markers are missing. Do not rerun.'
}

$SyntaxResult = Invoke-HamoonSsh -Command 'cd /root/Hamoon && node --check index.js && node --check cloud-api.js && node --check Hetzner/hetzner-api.js && echo R2_FINAL_NODE_CHECKS=SUCCESS' -Retries 8
...
Write-Host ''
Write-Host 'V10_R2_REMOTE_VERIFICATION=SUCCESS' -ForegroundColor Green
Write-Host 'HETZNER TRAFFIC + PROVIDER-404 v10 CRLF RESUME COMPLETED' -ForegroundColor Green
Write-Host "R2_REMOTE_LOG=$RunLogPath"
