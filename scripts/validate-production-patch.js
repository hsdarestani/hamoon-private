...
const free = read('index.js').split('async function handleFreeTrialRequest')[1]?.split('async function handleCycleSelection')[0]||'';
...
assert('cloud-init root password login', /ssh_pwauth: true/.test(read('index.js')) && /disable_root: false/.test(read('index.js')) && /PermitRootLogin yes/.test(read('index.js')) && /PasswordAuthentication yes/.test(read('index.js')) && /name: root/.test(read('index.js')));
