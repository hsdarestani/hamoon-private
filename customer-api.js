...
async function userPurchase(req, res, next) {
  const p = await db.getPurchaseForUserServer(req.apiClient.telegram_id, req.params.id, 'hetzner');
...
function createCustomerApiRouter() {
  ...
  router.get('/servers', async (req,res,next)=>{ try{const data=await db.listAdminServers({ userId:req.apiClient.telegram_id, limit:200 }); res.json({ ok:true, servers:data.rows.filter(r=>r.datacenter==='hetzner') });}catch(e){next(e);} });
  ...
  router.post('/servers', async (req,res,next)=>{ try{
    const client=req.apiClient; const dc=datacenters.hetzner; if(!dc) return apiError(res,503,'HETZNER_UNAVAILABLE','دیتاسنتر هتزنر فعال نیست.');
    const body=req.body||{}; const duration=['hourly','monthly'].includes(body.duration)?body.duration:'hourly';
    ...
    if(!isAllowed(client.allowed_datacenters,'hetzner')||!isAllowed(client.allowed_plans,plan.id)||!isAllowed(client.allowed_images,image)||!isAllowed(client.allowed_locations,location)) return apiError(res,403,'NOT_ALLOWED','این پلن، ایمیج یا لوکیشن برای این کلاینت مجاز نیست.');
    ...
    const ip=publicIpFromServer(srv); await db.recordPurchase(client.telegram_id, String(srv.id), 'hetzner', srv.name||name, plan.id, price, duration, 0, 0, null, 'api', image, 0,0,0,0,0, key_id, ip ? 'active' : 'pending_ssh');
  ...
  router.delete('/servers/:id', userPurchase, async (req,res,next)=>{ try{try{await cloud.deleteServer(datacenters.hetzner,null,req.params.id); await db.updatePurchaseStatus(req.params.id,'deleted');}catch(e){ if(e.response?.status===404||e.status===404){ await db.updatePurchaseStatus(req.params.id,'provider_missing'); } else throw e;} res.json({ ok:true });}catch(e){next(e);} });
  for (const [path,fn] of [['poweron','resumeServer'],['poweroff','suspendServer'],['reboot','rebuildServer']]) router.post(`/servers/:id/${path}`, userPurchase, async (req,res,next)=>{ try{ if(path==='reboot') return apiError(res,400,'UNSUPPORTED_ACTION','ریبوت مستقیم در این نسخه فعال نیست.'); await cloud[fn](datacenters.hetzner,null,req.params.id); res.json({ ok:true }); }catch(e){next(e);} });
  router.post('/servers/:id/upgrade', userPurchase, async (req,res,next)=>{ try{const plans=await getHetznerSellablePlans(datacenters.hetzner); const target=plans.find(p=>p.id===String(req.body?.target_server_type||'').toLowerCase()); if(!target)return apiError(res,400,'INVALID_PLAN','پلن هدف معتبر نیست.'); await cloud.changeHetznerServerType(datacenters.hetzner,req.params.id,target.hetzner_type,!!req.body?.upgrade_disk); await db.updatePurchasePlan(req.apiClient.telegram_id, req.params.id, 'hetzner', target.id, target.amount_monthly); res.json({ ok:true });}catch(e){next(e);} });
  router.use((err,_req,res,_next)=>{ console.error('[CUSTOMER_API_ERROR]', err.code || err.message); apiError(res,500,'INTERNAL_ERROR','خطای داخلی رخ داد.'); });
  return router;
}
