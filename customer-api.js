...
function createCustomerApiRouter() {
  ...
  router.post('/servers', async (req,res,next)=>{ try{
    ...
    const ip=publicIpFromServer(srv); await db.recordPurchase(client.telegram_id, String(srv.id), dcKey, srv.name||name, plan.id, price, duration, 0, 0, null, 'api', image, 0,0,0,0,0, key_id, 'provisioning');
    await db.recordWalletLog(client.telegram_id, 0, `API server create ${srv.id}`, 'server_api_create').catch(()=>{});
    res.status(202).json({ ok:true, operation:'provisioning', server:{ id:String(srv.id), name:srv.name||name, status:'provisioning', public_ip:null, server_type:plan.id, price } });
  }catch(e){next(e);} });
  router.delete('/servers/:id', userPurchase, async (req,res,next)=>{ try{ await lifecycle.deletePurchaseServer({ db, dc: datacenters[req.purchase.datacenter], telegramId: req.apiClient.telegram_id, serverId: req.params.id, datacenter: req.purchase.datacenter }); res.json({ ok:true, status:'deleted' }); }catch(e){next(e);} });
  ...
  router.post('/servers/:id/change-ip', userPurchase, async (req,res,next)=>{ try{ res.status(202).json({ ok:true, operation:'change_ip_started', status:'running' }); }catch(e){next(e);} });
